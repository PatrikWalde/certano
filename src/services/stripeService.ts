...
class StripeService {
  ...
  /**
   * Redirect to Stripe checkout (using server-side checkout sessions with User ID)
   */
  async redirectToCheckout(priceId: string, userId: string): Promise<void> {
    try {
      ...
      console.log('Creating server-side checkout session with User ID:', { priceId, userId });
